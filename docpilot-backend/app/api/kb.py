...
@router.get("/knowledge-bases/{kb_id}/documents/{document_id}/processing-status")
async def get_document_processing_status(
    ...
    task_status = (
        await document_task_service.get_latest_task_status(
            db=db,
            kb_id=kb_id,
            document_id=document_id,
        )
    ...
    if task_status is None:
        raise HTTPException(
            status_code=404,
            detail="Document processing task not found",
        )
