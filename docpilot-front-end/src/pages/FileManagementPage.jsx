...
import {
  ...
  matchesLocalUpload,
  normalizeDocument as normalizeServerDocument,
...
const STATUS_META = {
  ...
  indexing: { label: "向量化中", color: "#7c3aed", bg: "#ede9fe" },
...
function normalizeDocument(doc, kb) {
  return normalizeServerDocument(doc, { kbId: kb.id, kbName: kb.name });
}

function isBatchSynced(localRecords, documents) {
  return localRecords.every((localRecord) => documents.some((document) => (
    matchesLocalUpload(document, localRecord)
  )));
}
...
export default function FileManagementPage({ onNavigate }) {
  ...
  const fetchFiles = useCallback(async ({ cleanupSyncedUploads = true, silent = false } = {}) => {
    ...
    try {
      ...
      if (isMountedRef.current) {
        setKbs(kbList);
        if (kbList[0]) setUploadKbId((current) => current || kbList[0].id);
      }
      ...
      if (isMountedRef.current) {
        ...
        if (cleanupSyncedUploads) {
          setUploadQueue((prev) => prev.filter((item) => (
            item.status === "failed" || !allFiles.some((document) => (
              matchesLocalUpload(document, item)
            ))
          )));
        }
        if (failedCount > 0) {
          setError("部分知识库的文件暂时无法加载。");
        }
      }

      return { success: failedCount === 0, files: allFiles };
    } catch (err) {
  ...
  const displayFiles = useMemo(() => {
    const unmatchedLocalUploads = uploadQueue.filter((localUpload) => (
      localUpload.status === "failed" || !files.some((serverDocument) => (
        matchesLocalUpload(serverDocument, localUpload)
      ))
    ));
    return [...files, ...unmatchedLocalUploads];
  }, [uploadQueue, files]);

  const hasActiveUploads = useMemo(() => {
    const isActive = (file) => ["processing", "indexing", "pending"].includes(file.status);
    return uploading || uploadQueue.some(isActive) || files.some(isActive);
  }, [files, uploading, uploadQueue]);
  ...
  useEffect(() => {
    ...
    const refreshStatus = async () => {
      ...
      if (!cancelled && isMountedRef.current) {
        statusPollTimerRef.current = window.setTimeout(refreshStatus, 2000);
      }
    };
  ...
  const stats = useMemo(() => {
    return {
      ...
      completed: displayFiles.filter((file) => file.status === "completed").length,
      processing: displayFiles.filter((file) => ["processing", "indexing", "pending"].includes(file.status)).length,
      failed: displayFiles.filter((file) => file.status === "failed").length,
    };
  }, [displayFiles]);
  ...
  const uploadFiles = async (fileList) => {
    ...
    setUploadQueue((prev) => [...prev, ...localRecords]);
    setUploading(true);

    try {
      const uploadResult = await uploadKnowledgeBaseFile(uploadKbId, selected);
      const unmatchedUploadResults = Array.isArray(uploadResult)
        ? [...uploadResult]
        : Array.isArray(uploadResult?.items)
          ...
          : uploadResult
            ? [uploadResult]
            : [];
      const confirmedLocalRecords = localRecords.map((localRecord) => {
        const uploadedDocument = findUploadedDocument(unmatchedUploadResults, localRecord);
        if (uploadedDocument) {
          unmatchedUploadResults.splice(unmatchedUploadResults.indexOf(uploadedDocument), 1);
        }
        return {
          ...localRecord,
      ...
      const confirmedRecordsById = new Map(
        confirmedLocalRecords.map((localRecord) => [localRecord.id, localRecord])
      );
      if (isMountedRef.current) {
        setUploadQueue((prev) => prev.map((item) => (
          confirmedRecordsById.get(item.id) || item
        )));
      }

      const refreshResult = await fetchFiles({ cleanupSyncedUploads: false, silent: true });
      const synced = refreshResult.success && isBatchSynced(
        confirmedLocalRecords,
        refreshResult.files
      );
  ...
  const handleRemoveLocalUpload = (fileId) => {
                  ...
                  <table className="fm-table">
                    ...
                    <tbody>
                      {paginatedFiles.map((file) => {
                        ...
                        return (
                          <tr key={file.isLocalUpload ? "local-" + file.id : file.kbId + "-" + file.id}>
                            <td>
                            ...
                            <td>
                              <div className="fm-table-actions">
                                {file.isLocalUpload ? (
                                  file.status === "failed" ? (
                                    <button className="fm-tbl-btn fm-tbl-btn--danger" title={file.uploadError} onClick={() => handleRemoveLocalUpload(file.id)}>移除记录</button>
                                  ) : (
                                    <span className="fm-cell-muted">{file.status === "processing" ? "正在上传" : "等待同步"}</span>
                                  )
                                ) : (
                                  <>
            ...
            <div className="fm-card" style={{ marginTop: 16 }}>
              ...
              <div className="fm-recent-list">
                {recentUploads.length === 0 ? (
                  ...
                  const statusMeta = STATUS_META[file.status] || STATUS_META.pending;
                  return (
                    <div key={file.isLocalUpload ? "recent-local-" + file.id : "recent-" + file.kbId + "-" + file.id} className="fm-recent-item" title={file.isLocalUpload && file.status === "failed" ? file.uploadError : undefined}>
