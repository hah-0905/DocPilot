...
import { findUploadedDocument, matchesLocalUpload, normalizeDocument } from "../utils/documentStatus";
...
export default function KnowledgeBaseDetailPage({ onNavigate }) {
  ...
  const fetchDocuments = useCallback(async (options = {}) => {
    const { cleanupSyncedUploads = true, silent = false } = options;
    if (!kbId) return { success: false, docs: [] };
    ...
    try {
      ...
      const mapped = Array.isArray(data)
        ? data.map((document) => normalizeDocument(document, { kbId }))
        : [];

      if (isMountedRef.current) {
        setDocs(mapped);
        if (cleanupSyncedUploads) {
          setUploadQueue((prev) => prev.filter((item) => (
            item.status === "failed" || !mapped.some((document) => (
              matchesLocalUpload(document, item)
            ))
          )));
        }
      }
      ...
      if (!silent && isMountedRef.current) setDocLoading(false);
    }
  }, [kbId]);
  ...
  const handleUpload = () => {
    ...
    input.onchange = async (event) => {
      ...
      const localRecord = {
        ...
        kbId,
        name: file.name,
        ext: nameParts.length > 1 ? nameParts.pop() : "",
        status: "processing",
        uploadedAt,
        chunkCount: "--",
        size: file.size,
      ...
      setUploadQueue((prev) => [...prev, localRecord]);
      setUploading(true);
      try {
        const uploadResult = await uploadKnowledgeBaseFile(kbId, file);
        if (!isMountedRef.current) return;
        const uploadedDocument = findUploadedDocument(uploadResult, localRecord);
        const confirmedLocalRecord = {
          ...localRecord,
          serverDocumentId: uploadedDocument?.id != null ? String(uploadedDocument.id) : null,
          sha256: uploadedDocument?.sha256 || "",
          size: uploadedDocument?.size_bytes ?? localRecord.size,
          allowFallbackMatch: true,
        };
        setUploadQueue((prev) => prev.map((item) => (
          item.batchId === batchId ? confirmedLocalRecord : item
        )));

        const finalRefresh = await fetchDocuments({ cleanupSyncedUploads: true, silent: true });
        if (!isMountedRef.current) return;
        const matchedDocument = finalRefresh.success && finalRefresh.docs.find((document) => (
          matchesLocalUpload(document, confirmedLocalRecord)
        ));

        if (matchedDocument) {
          setUploadQueue((prev) => prev.filter((item) => item.batchId !== batchId));
        } else {
          setUploadQueue((prev) => prev.map((item) => (
            item.batchId === batchId ? { ...item, status: "pending", uploadError: "" } : item
          )));
        }
      } catch (err) {
        if (!isMountedRef.current) return;
        const uploadError = err.message || "上传失败";
        setUploadQueue((prev) => prev.map((item) => (
          item.batchId === batchId ? { ...item, status: "failed", uploadError } : item
        )));
        alert(uploadError);
      } finally {
  ...
  const handleRemoveLocalUpload = (id) => {
  ...
  const displayDocs = useMemo(() => {
    const unmatchedLocalUploads = uploadQueue.filter((localUpload) => (
      localUpload.status === "failed" || !docs.some((serverDocument) => (
        matchesLocalUpload(serverDocument, localUpload)
      ))
    ));
    return [...docs, ...unmatchedLocalUploads];
  }, [uploadQueue, docs]);

  const hasActiveTasks = useMemo(() => {
    const hasLocalTask = uploadQueue.some((item) => (
      item.status === "processing" || item.status === "pending"
    ));
    const hasServerTask = docs.some((document) => (
      document.status === "processing" || document.status === "indexing" || document.status === "pending"
    ));
    return uploading || hasLocalTask || hasServerTask;
  }, [uploading, uploadQueue, docs]);
  ...
  useEffect(() => {
    ...
    const scheduleNextRefresh = async () => {
      ...
      await fetchDocuments({ cleanupSyncedUploads: true, silent: true });
      if (!cancelled && isMountedRef.current) {
        statusPollTimerRef.current = window.setTimeout(scheduleNextRefresh, 2000);
      }
    };
  ...
  }, [hasActiveTasks, fetchDocuments]);

  const stats = useMemo(() => {
    const fileCount = displayDocs.length;
    const chunkCount = displayDocs.reduce((sum, doc) => (
      sum + (typeof doc.chunkCount === "number" ? doc.chunkCount : 0)
    ), 0);
    const latestUpdated = displayDocs.reduce((latest, doc) => (
  ...
  const task = useMemo(() => {
    const isActiveDocument = (document) => ["processing", "indexing", "pending", "failed"].includes(document.status);
    const newestFirst = (left, right) => String(right.createdAt || right.uploadedAt || "").localeCompare(String(left.createdAt || left.uploadedAt || ""));
    const serverActiveDocument = docs.filter(isActiveDocument).slice().sort(newestFirst)[0];
    const localActiveUpload = uploadQueue.filter(isActiveDocument).slice().sort(newestFirst)[0];
    const activeDocument = serverActiveDocument || localActiveUpload || null;
    if (!activeDocument) return null;

    const parseSucceeded = ["success", "completed", "indexed", "vectorized"].includes(activeDocument.parseStatus);
    const indexSucceeded = ["indexed", "completed", "success", "vectorized"].includes(activeDocument.indexStatus) || activeDocument.status === "completed";
    const parseFailed = ["failed", "error"].includes(activeDocument.parseStatus);
    const indexFailed = ["failed", "error"].includes(activeDocument.indexStatus);
    const isIndexing = activeDocument.status === "indexing";
    const failed = activeDocument.status === "failed";

    return {
      fileName: activeDocument.name,
      uploadedAt: activeDocument.createdAt || activeDocument.uploadedAt,
      size: activeDocument.size,
      message: activeDocument.uploadError || "",
      isFailed: failed || parseFailed || indexFailed,
      parseStatus: failed || parseFailed ? "failed" : parseSucceeded ? "success" : activeDocument.status === "processing" ? "processing" : "pending",
      splitStatus: failed || parseFailed ? "pending" : parseSucceeded ? "success" : "pending",
      embeddingStatus: indexFailed ? "failed" : indexSucceeded ? "success" : isIndexing ? "processing" : "pending",
    };
  }, [docs, uploadQueue]);
          ...
          <div className="kb-detail-body">
            ...
            <div className="kb-detail-table-wrapper">
              <div className="kb-detail-table-header">
                <h3>知识库文件（{displayDocs.length}）</h3>
              </div>

              {docLoading && displayDocs.length === 0 ? (
                <div className="kb-skeleton" style={{ height: 200 }} />
              ) : docError && displayDocs.length === 0 ? (
                  ...
                  <table className="kb-detail-table">
                    ...
                    <tbody>
                      {displayDocs.map((doc) => {
                        ...
                        return (
                          <tr key={doc.isLocalUpload ? "local-" + doc.id : doc.id}>
                            <td>
                            ...
                            <td>
                              ...
                              >
{doc.isLocalUpload && status === "failed" ? "上传失败" : STATUS_LABEL[status] || status}
                              </span>
                            </td>
                            ...
                            <td>
                              <div className="kb-table-actions">
                                {doc.isLocalUpload ? (
                                  status === "failed" ? (
                                    <button className="kb-table-action-btn kb-table-action-btn--danger" title={doc.uploadError} onClick={() => handleRemoveLocalUpload(doc.id)}>移除记录</button>
                                  ) : (
                                    <span className="kb-cell-muted">{status === "processing" ? "正在上传" : "等待同步"}</span>
                                  )
                                ) : (
                                  <>
              ...
              <div className="kb-pagination">
                <span className="kb-pagination__info">共 {displayDocs.length} 条</span>
            ...
            <div className="kb-detail-task-panel">
              ...
              {task ? (
                <div className="kb-task-panel__content">
                  ...
                  <StageRow label="解析文件" status={task.parseStatus} detail={formatSize(task.size)} />
                  <StageRow label="切分文本" status={task.splitStatus} />
                  <StageRow label="向量化" status={task.embeddingStatus} />
                  <div className="kb-task-note">
                    <SvgIcon name="loader" size={14} />
<span>{task.isFailed ? "任务处理失败" : task.message || "任务进行中，离开此页不影响执行"}</span>
                  </div>
                </div>
              ) : (
...
<span className={`kb-task-stage__pct ${isSuccess ? "kb-task-stage__pct--done" : ""}`}>{isSuccess ? "已完成" : isProcessing ? "处理中" : "等待中"}</span>
      </div>
